...
class Sensor:
    ...
    def __check_new_tags(self,reading_name,units):
    ...
    def add_data(self, timestamp, reading, units, value, theme, extra):
        ...
        insert_string = "insert into sensor_data (info) values ('%s')"\
                                                        % (','.join(hstore),)
        self.database.insert(insert_string)
        self.__check_new_tags(reading,units)
    ...
    def add_readings_from_dict(self,readings_dict):
        for name,info in readings_dict.iteritems():
            if 'extra' not in info.keys():
                info['extra'] ={}
            self.add_data(info['timestamp'],name,info['units'],info['value'],info['theme'],info['extra'])
